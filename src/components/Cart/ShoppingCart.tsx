...
const ShoppingCart = () => {
  ...
  const [cartDetails, setCartDetails] = useState<ICartCourse[]>([]);
    ...
    const getInstructorData = async () => {
      try {
        ...
        if (data.success == true) {
          setCartDetails(data.cartCourse);
          setLoading(false);
        } else {
  ...
  for (let i = 0; i < cartDetails.length; i++) {
    totalOriginalPrice = totalOriginalPrice + cartDetails[i]?.original_price;
    totalDiscountedPrice =
      totalDiscountedPrice + cartDetails[i]?.discounted_price;
  }
        ...
        <main className="cart container p-4">
          <div className="row">
            ...
            <h3 className="fw-bold fs-4 mt-4 mb-3">
              {cartDetails.length} Courses in Cart
            </h3>
